<template>
  ...
  >
    <!-- Top accent bar -->
    <div class="h-1.5 bg-gray-200 dark:bg-dark-700" />
    ...
    <div class="flex flex-1 flex-col p-4">
      ...
      <div class="mb-3 grid grid-cols-2 gap-x-3 gap-y-1 rounded-lg bg-gray-50 px-3 py-2 text-xs dark:bg-dark-700/50">
        ...
        <div v-if="hasPeakRate" class="col-span-2 flex items-center justify-between gap-2">
          ...
          <span class="text-right font-medium text-gray-700 dark:text-gray-300">{{ peakRateDisplay }}</span>
        </div>
      ...
      <div v-if="plan.features.length > 0" class="mb-3 space-y-1">
        <div v-for="feature in plan.features" :key="feature" class="flex items-start gap-1.5">
          <svg class="mt-0.5 h-3.5 w-3.5 flex-shrink-0 text-gray-400 dark:text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
      ...
      <button
        type="button"
        class="btn btn-primary w-full rounded-xl py-2.5 text-sm font-semibold transition-all active:scale-[0.98]"
